...
;(function()
{
	 ...
	 */
	EeWalk11.Dropbox.Draggable = function(text, options)
	{
		//Check for jQuery
		...
		var $elem = jQuery(
			"<div id='jsdb-draggable-" + thisID + "' class='jsdb-draggable' draggable='true'>"
				+ text + "</div>"
		);

		//Create a private data object for this instance
		draggableData.push(new EeWalk11.Dropbox.DraggableData($elem, text, options));
	...
	/**
	 * Get the data to transfer with the drag and drop event.
	 * <p>By default the Draggable element's text will be transferred.</p>
	...
	/**
	 * Get a string used for sorting this Draggable element in a Container.
	 * <p>By default, the text will be returned. If different behavior is needed, this method can be
	 * overridden by a child prototype.</p>
	 ...
	 * @type {Number} The next Draggable's ID.
	 */
	var draggableID = 0;

	/**
	 * @type {Object} Stores objects with private data for each Draggable instance.
	 */
	var draggableData = [];
	...
	/**
	 * @type {Array} An array of all Draggable instances.
	 */
	var draggables = [];
	 ...
	 * @param {Number|HTMLElement|jQuery|EeWalk11.Dropbox.Draggable} arg0 A Draggable element's
	 * numeric ID or the element's DOM element/element ID. The Draggable element itself can also be
	 * passed, in which case it will simply be returned.
	...
	/**
	 * Apply hover animations to this Draggable element.
	 * <p>If hover animation options were not provided in the constructor, this method will do
	 * nothing.</p>
	 */
