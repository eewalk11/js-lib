...
;(function()
{
	 ...
	 * @param {jQuery|String} element The element that will act as the Container. This can be a
	 * jQuery object with length 1 or an element ID.
	 ...
	 */
	EeWalk11.Dropbox.Container = function(element, options)
	{
		...
		var $elem = EeWalk11.toJQuery(element);
		if(!$elem)
		{
			throw new Error("Invalid dropbox container: " + element);
		}

		//Create a private data object for this instance
		containerData.push(new EeWalk11.Dropbox.ContainerData($elem, options));
	...
	/**
	 * @type {Function} Constructor alias.
	 */
	...
	/**
	 * Add a Draggable element to this Container and the DOM.
	 * <p>If this container is sortable, the element will be appended to the Container. Otherwise,
	 * the element will be inserted into its sorted position. If the element already exists in a
	 * Container, or this Container is full, nothing will be added.</p>
	 * @param {EeWalk11.Dropbox.Draggable} draggable The Draggable element to add to this Container.
	 * @return {Boolean} True if the element was added, false otherwise.
	 */
	EeWalk11.Dropbox.Container.prototype.add = function(draggable)
	{
		if(
			draggable instanceof EeWalk11.Dropbox.Draggable
			&& !Container.getContainer(draggable) && !this.isFull()
		)
		{
			return this.isSortable() ?
				appendDraggable.call(this, draggable)
	 ...
	 * @return {Boolean} True if the element is in this Container, false otherwise.
	 */
	EeWalk11.Dropbox.Container.prototype.contains = function(draggable)
	{
		if(draggable instanceof EeWalk11.Dropbox.Draggable)
		{
			...
			for(var i = 0, len = data.draggables.length; i < len; i++)
			{
	...
	/**
	 * Determine if a variable is an equivalent Container.
	 * <p>By default, the unique Container IDs will be compared.</p>
	 ...
	 * @returns {EeWalk11.Dropbox.Draggable|Boolean} The Draggable element, false on failure.
	 */
	...
	/**
	 * Get the position of a Draggable element in this Container.
	 * <p>If the element exists in this Container, its current position will be returned. If the
	 * element does not exist in this Container and this container is sortable, true will be
	 * returned. If the element does not exist in this Container and this Container is not sortable,
	 * the position of the element after adding and sorting it will be returned. If the element does
	 * not exist in this Container and the container is full, false will be returned.</p>
	 * @param {EeWalk11.Dropbox.Draggable} draggable The draggable element.
	 * @return {Number|Boolean} The Draggable element's [expected] position, true if can be added to
	 * a custom position, false if it cannot be added.
	 */
	...
	/**
	 * Add a Draggable element to this Container and the DOM at a given position.
	 * <p>If the element already exists in a Container, the index is invalid, or this Container is
	 * full, nothing will be inserted.</p>
	 * @param {EeWalk11.Dropbox.Draggable} draggable The Draggable element to insert into this
	 * Container.
	 * @param {Number} index The index at which to insert the Draggable element.
	...
	EeWalk11.Dropbox.Container.prototype.insert = function(draggable, index)
	{
		...
		)
		{
			...
			return index === 0 ?
				prependDraggable.call(this, draggable)
	...
	* elements within this Container will be automatically sorted as defined by the Draggable
	* prototype's compare function.
	*/
   ...
   /**
	 ...
	 * this Container, or its position (index).
	...
	/**
	 * Set a fixed height for this Container.
	 * <p>This function's automated behavior (height is undefined) will set the height to the
	 * minimum that the Container needs to be fit all Draggable elements when at capacity. This will
	 * only work if there is a capacity set for this Container and there is at least 1 Draggable
	 * element added to the DOM.</p>
	 * @param {Number} height The height of the Container (in pixels). Leave undefined for automated
	 * height setting.
	 ...
	 * @type {Number} The next Container's ID.
	 */
	var containerID = 0;

	/**
	 * @type {Object} Stores objects with private data for each Container instance.
	 */
	var containerData = [];
	...
	/**
	 * @type {Array} An array of all Container instances.
	 */
	var containers = [];
	 ...
	 * @param {Number|String|HTMLElement|jQuery|EeWalk11.Dropbox.Container} arg0 A Container's
	 * numeric ID, the Container's DOM element/element ID, or a Draggable element that is in the
	 * Container. The Container itself can also be passed, in which case it will simply be returned.
	 * @return {EeWalk11.Dropbox.Container|Boolean} The Container, false on failuer.
	 */
	...
	/**
	 * Set a fixed height for each Container instance.
	 * <p><b>Only use this function if all Containers should be the same height.</b></p>
	 * @param {Number} height The height (in pixels). Leave undefined for automated height setting.
	 ...
	 * <p>Called as a container method.</p>
	 */
	...
	/**
	 * Get a Container from a Draggable element.
	 * <p>The returned Container is the one that contains the given Draggable element.</p>
	 * @param {EeWalk11.Dropbox.Draggable} draggable The Draggable element.
	 ...
	 * @param {EeWalk11.Dropbox.Draggable} draggable The draggable element.
	 * @param {Number} index The insert position (1 or greater - use appendDraggable for index 0).
	 * @return {Boolean} True.
	 */
